...
class NavAdmin extends AdminController
{
    ...
    public function index($group = 'main')
    {
        ...
        foreach ($data_list as &$val) {
            if ($val['type'] == 'post') {
                $val['title'] = '<a href="' . U('Git/Post/index', array('cid' => $val['id'])) . '">' . $val['title'] . '</a>';
            }
        }
    ...
    public function add($group)
    {
        if (IS_POST) {
            ...
            $builder->setMetaTitle('新增导航') // 设置页面标题
                ...
                ->addFormItem('pid', 'select', '上级导航', '上级导航', select_list_as_tree('Git/Nav', array('group' => $group), '顶级导航'))
                ->addFormItem('title', 'text', '导航标题', '导航前台显示标题')
                ->addFormItem('type', 'radio', '导航类型', '导航类型', D('Git/Nav')->nav_type())
